...
public static class SystemServiceExports
{
    ...
    [SysAbiExport(Nid = "656LMQSrg6U", ExportName = "sceSystemServiceReceiveEvent", Target = Generation.Gen5, LibraryName = "libSceSystemService")]
    public static int SystemServiceReceiveEvent(CpuContext ctx)
    {
        var eventAddress = ctx[CpuRegister.Rdi];
        if (eventAddress == 0) return ctx.SetReturn(OrbisSystemServiceErrorParameter);
        Span<byte> systemEvent = stackalloc byte[SystemServiceEventSize];
        systemEvent.Clear();
        BinaryPrimitives.WriteInt32LittleEndian(systemEvent, -1);
        return ctx.Memory.TryWrite(eventAddress, systemEvent) ? ctx.SetReturn(OrbisSystemServiceErrorNoEvent) : ctx.SetReturn((int)OrbisGen2Result.ORBIS_GEN2_ERROR_MEMORY_FAULT);
    }

    [SysAbiExport(Nid = "8Lo6Zv94aho", ExportName = "sceSystemServiceDisableNoticeScreenSkipFlagAutoSet", Target = Generation.Gen5, LibraryName = "libSceSystemService")]
    public static int SystemServiceDisableNoticeScreenSkipFlagAutoSet(CpuContext ctx) => ctx.SetReturn(0);

    [SysAbiExport(Nid = "Q3utJvma4Mo", ExportName = "sceSystemServiceSetNoticeScreenSkipFlag", Target = Generation.Gen5, LibraryName = "libSceSystemService")]
    public static int SystemServiceSetNoticeScreenSkipFlag(CpuContext ctx) => ctx.SetReturn(0);

    [SysAbiExport(Nid = "XbbJC3E+L5M", ExportName = "sceSystemServicePowerTick", Target = Generation.Gen5, LibraryName = "libSceSystemService")]
